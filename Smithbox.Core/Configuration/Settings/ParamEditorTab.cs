...
public class ParamEditorTab
{
    ...
    public void Display()
    {
        ...
        if (ImGui.CollapsingHeader("Rows", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            UIHelper.Tooltip("Allow the row order to be changed by an alternative order as defined within the PARAM META file.");
        }
        ...
        if (ImGui.CollapsingHeader("Row Context Menu", ImGuiTreeNodeFlags.DefaultOpen))
        {
            ...
            UIHelper.Tooltip("Show the copy id option in the right-click row context menu.");
        }
        ...
        {
            if (Smithbox.ProjectType != ProjectType.None)
            {
