...
namespace Smithbox.Core.Utilities;

public static class ModEngineLauncher
{
    /// <summary>
    /// A bit hacky, but good enough for now
    /// </summary>
    /// <param name="curProject"></param>
    public static void LaunchMod(ProjectInstance curProject)
    {
...
enabled = false";
        ...
        if (File.Exists(modTomlPath))
        {
            ...
            {
                PlatformUtils.Instance.MessageBox("Steam is not currently running. Start Steam.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
    }
}
