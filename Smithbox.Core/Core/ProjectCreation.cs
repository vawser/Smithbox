        ...
        if (Display)
        {
            if (ImGui.Begin("Project Creation##projectCreationWindow", ref Display, ImGuiWindowFlags.NoResize | ImGuiWindowFlags.NoMove))
            {
                if (ImGui.BeginTable($"projectCreationTable", 3, ImGuiTableFlags.SizingFixedFit))
                {
                    ...
                    {
                        var selected = PlatformUtils.Instance.OpenFolderDialog("Select Project Folder", out string newProjectPath);
                    ...
                    {
                        var selected = PlatformUtils.Instance.OpenFolderDialog("Select Data Folder", out string newDataPath);
