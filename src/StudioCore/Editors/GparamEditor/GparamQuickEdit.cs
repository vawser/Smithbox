...
public class GparamQuickEdit
{
    ...
    public GparamQuickEdit(Project curPoject, GparamEditor editor)
    {
        Editor = editor;
        Project = curPoject;
    ...
    public void ExecuteQuickEdit()
    {
        ...
        KeyValuePair<string, GPARAM> TargetGraphicsParam = new();

        for (int k = 0; k < Project.GparamData.PrimaryBank.GraphicsParams.Count; k++)
        {
            var curEntry = Project.GparamData.PrimaryBank.GraphicsParams.ElementAt(k);

            if (IsTargetGraphicsParam(curEntry.Key))
            {
                TargetGraphicsParam = curEntry;
                break;
            }
        }

        if (TargetGraphicsParam.Value == null)
            return;

        for (int i = 0; i < TargetGraphicsParam.Value.Params.Count; i++)
        {
            GPARAM.Param entry = TargetGraphicsParam.Value.Params[i];
    ...
    public bool IsTargetGraphicsParam(string name)
    {
        var match = false;
        ...
        foreach (var command in commands)
        {
            ...
            {
                if (Editor.Selection._selectedGparamKey == name)
                {
                    match = true;
                    continue;
                }
            ...
            if (filterMatch.Success && filterMatch.Groups.Count >= 2)
            {
                ...
                if (commandValue == name || commandValue == "*")
                {
                    match = true;
                }
            }
    ...
    private (bool, object) GetVanillaValue(int index)
    {
        ...
        var gparamName = Editor.Selection._selectedGparamKey;

        if (!Project.GparamData.VanillaBank.GraphicsParams.ContainsKey(gparamName))
            return (false, -1);

        var vanillaEntry = Project.GparamData.VanillaBank.GraphicsParams[gparamName];

        foreach (var paramGroup in vanillaEntry.Params)
        {
