...
public class EventScriptDecorator
{
    ...
    public EventScriptDecorator(Project curProject, EventScriptEditor editor)
    {
        Project = curProject;
        Editor = editor;
    }
    ...
    public void DetermineParamReference(string parameterName, string value, int i)
    {
        ...
        var currentEventID = Editor.Selection.SelectedEvent.ID.ToString();
    ...
    {
        var mapID = Editor.Selection.SelectedFileKey;
    ...
    {
        var enumDoc = Editor.Project.EventScriptData.EventInformation.Enums.Where(e => e.Name == argDoc.EnumName).FirstOrDefault();
    ...
    private void ConstructParamReference(string paramName, string value, int i)
    {
        ...
        (string, Param.Row, string) match = ResolveParamRef(Editor.BaseEditor.ParamData.PrimaryBank, paramName, refValue);
    ...
    {
        if (Editor.BaseEditor.TextData == null)
            return;
    ...
    /// </summary>
    private (string, Param.Row, string) ResolveParamRef(string paramRef, dynamic oldval)
    {
        (string, Param.Row, string) row = new();

        if (Editor.BaseEditor.ParamData == null)
            return row;

        if (Editor.BaseEditor.ParamData.PrimaryBank == null)
            return row;

        if (Editor.BaseEditor.ParamData.PrimaryBank.Params == null)
            return row;

        var originalValue = (int)oldval; //make sure to explicitly cast from dynamic or C# complains. Object or Convert.ToInt32 fail.

        var hint = "";
        if (Editor.BaseEditor.ParamData.PrimaryBank.Params.ContainsKey(paramRef))
        {
            var altval = originalValue;

            Param param = Editor.BaseEditor.ParamData.PrimaryBank.Params[paramRef];

            var meta = Editor.BaseEditor.ParamData.Meta.Get(Editor.BaseEditor.ParamData.PrimaryBank.Params[paramRef].AppliedParamdef);
            ...
            if (r == null && altval > 0 && meta != null)
            {
                ...
                r = Editor.BaseEditor.ParamData.PrimaryBank.Params[paramRef][altval];
            }
