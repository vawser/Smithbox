    ...
    public static class DummyGroups
    {
        ...
        public static void DisplayConfiguration(ModelEditorScreen screen)
        {
            ...
            if (ImGui.Button("Create Dummy Group", defaultButtonSize))
            {
                if(screen.ModelHierarchy._selectedDummy != -1 || 
                    screen.ModelHierarchy.DummyMultiselect.StoredIndices.Count > 0)
                {
        ...
        public static void CreateDummyGroup(ModelEditorScreen screen, string filename)
        {
            ...
            for (int i = 0; i < screen.ResourceHandler.CurrentFLVER.Dummies.Count; i++)
            {
                ...
                {
                    for (int j = 0; j < screen.ModelHierarchy.DummyMultiselect.StoredIndices.Count; j++)
                    {
                        if (j == i)
                        {
                ...
                {
                    if (i == screen.ModelHierarchy._selectedDummy)
                    {
                        newDummyList.List.Add(curDummy);
                    }
                }
