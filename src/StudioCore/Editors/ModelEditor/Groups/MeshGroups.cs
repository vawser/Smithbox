    ...
    public static class MeshGroups
    {
        ...
        public static void DisplayConfiguration(ModelEditorScreen screen)
        {
            ...
            if (ImGui.Button("Create Mesh Group", defaultButtonSize))
            {
                if (screen.ModelHierarchy._selectedMesh != -1 ||
                    screen.ModelHierarchy.MeshMultiselect.StoredIndices.Count > 0)
                {
        ...
        public static void CreateMeshGroup(ModelEditorScreen screen, string filename)
        {
            ...
            for (int i = 0; i < screen.ResourceHandler.GetCurrentFLVER().Meshes.Count; i++)
            {
                ...
                {
                    for (int j = 0; j < screen.ModelHierarchy.MeshMultiselect.StoredIndices.Count; j++)
                    {
                        if (j == i)
                        {
                ...
                {
                    if (i == screen.ModelHierarchy._selectedMesh)
                    {
                        newMeshList.List.Add(curMesh);
                    }
                }
