    ...
    public static class NodeGroups
    {
        ...
        public static void DisplayConfiguration(ModelEditorScreen screen)
        {
            ...
            if (ImGui.Button("Create Node Group", defaultButtonSize))
            {
                if (screen.ModelHierarchy._selectedNode != -1 ||
                    screen.ModelHierarchy.NodeMultiselect.StoredIndices.Count > 0)
                {
        ...
        public static void CreateNodeGroup(ModelEditorScreen screen, string filename)
        {
            ...
            for (int i = 0; i < screen.ResourceHandler.CurrentFLVER.Nodes.Count; i++)
            {
                ...
                {
                    for (int j = 0; j < screen.ModelHierarchy.NodeMultiselect.StoredIndices.Count; j++)
                    {
                        if (j == i)
                        {
                ...
                {
                    if (i == screen.ModelHierarchy._selectedNode)
                    {
                        newNodeList.List.Add(curNode);
                    }
                }
