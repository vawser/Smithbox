...
namespace StudioCore.Editors.ModelEditor.Tools
{
    ...
    public static class MaterialGroups
    {
        ...
        public static void DisplayConfiguration(ModelEditorScreen screen)
        {
            ...
            ImguiUtils.WrappedText("Create a stored Material Group from your current selection with the Material list.");
            ImguiUtils.WrappedText("A stored group can then be used to replace the existing Material list, or appended to the end.");
            ...
            if (ImGui.Button("Create Material Group", defaultButtonSize))
            {
                if (screen.ModelHierarchy._selectedMaterial != -1 ||
                    screen.ModelHierarchy.MaterialMultiselect.StoredIndices.Count > 0)
                {
            ...
            foreach (var entry in MaterialGroupFiles)
            {
                ...
                if (_selectedMaterialGroup == entry)
                {
                    if (ImGui.BeginPopupContextItem($"##MaterialSelectionPopup{entry}"))
                    {
                        ...
                        ImguiUtils.ShowHoverTooltip("Delete this Material group.");
            ...
            if (_selectedMaterialGroup != "" && SelectedMaterialList != null)
            {
                ...
                ImguiUtils.ShowHoverTooltip("Replace the existing Materials with the Materials within this Material group.");
                ImGui.SameLine();
                ...
                ImguiUtils.ShowHoverTooltip("Append to Materials within this Material group to the existing Materials.");
            }
        ...
        private static void DisplayCreationModal(ModelEditorScreen screen)
        {
            ...
            ImguiUtils.ShowHoverTooltip("The name of the Material group.");
        ...
        public static void CreateMaterialGroup(ModelEditorScreen screen, string filename)
        {
            ...
            for (int i = 0; i < screen.ResourceHandler.CurrentFLVER.Materials.Count; i++)
            {
                ...
                {
                    for (int j = 0; j < screen.ModelHierarchy.MaterialMultiselect.StoredIndices.Count; j++)
                    {
                        if (j == i)
                        {
                ...
                {
                    if (i == screen.ModelHierarchy._selectedMaterial)
                    {
                        newMaterialList.List.Add(curMaterial);
                    }
                }
