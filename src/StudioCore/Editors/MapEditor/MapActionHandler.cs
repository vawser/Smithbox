...
public class MapActionHandler
{
    ...
    public void ApplyDelete()
    {
        if (Editor.Selection.IsSelection())
        {
            DeleteMapObjectsAction action = new(Editor, Editor.Selection.GetFilteredSelection<MsbEntity>().ToList(), true);
            Editor.EditorActionManager.ExecuteAction(action);
        }
        else
        {
    ...
    public void ApplyMoveToCamera()
    {
        if (Editor.Selection.IsSelection())
        {
            ...
            Vector3 camDir = Vector3.Transform(Vector3.UnitZ, Editor.MapViewport.WorldView.CameraTransform.RotationMatrix);
            Vector3 camPos = Editor.MapViewport.WorldView.CameraTransform.Position;
