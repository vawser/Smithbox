...
public class MapPropertyView
{
    ...
    private string WindowName = "Properties##mapEditorPropertyView";
    ...
    public void Display()
    {
        ...
        Editor.EditorFocus.SetFocusContext(MapEditorContext.MapObjectProperties);

        HashSet<Entity> entSelection = Editor.Selection.GetFilteredSelection<Entity>();
        ...
        if (Editor.Universe.HasProcessedMapLoad && entSelection.Count > 1)
        {
            ...
            if (firstEnt.WrappedObject is Param.Row prow || firstEnt.WrappedObject is MapParamRow)
            {
            ...
            PropEditorSelectedEntities(Editor.Selection);
            ImGui.PopStyleColor();
            ImGui.EndChild();
        }
    ...
    private void PropEditorParamRow(Entity selection)
    {
        ...
        else if (selection.WrappedObject is MapParamRow mrow)
        {
        ...
        {
            if (selection.WrappedObject is MapParamRow)
            {
                var mergedRow = (MapParamRow)selection.WrappedObject;
    ...
    private void DisplayPropContextMenu(MapEntityPropertyFieldMeta meta, ViewportSelection selection, PropertyInfo prop, object obj, int arrayIndex)
    {
        ...
        if (ImGui.BeginPopup("MsbPropContextMenu"))
        {
            ...
            if (meta != null && meta.PositionProperty)
            {
                ...
                {
                    Editor.MapEditorDecorator.CopyCurrentPositionValue(prop, obj);
                }
                if (ImGui.Selectable(@"Paste##PastePosition"))
                {
                    Editor.MapEditorDecorator.PasteCurrentPositionValue();
                }
            }
            ...
            if (meta != null && meta.RotationProperty)
            {
                ...
                {
                    Editor.MapEditorDecorator.PasteCurrentRotationValue();
                }
            }
            ...
            if (meta != null && meta.ScaleProperty)
            {
                ...
                {
                    Editor.MapEditorDecorator.CopyCurrentScaleValue(prop, obj);
                }
                if (ImGui.Selectable(@"Paste##PasteScale"))
                {
                    Editor.MapEditorDecorator.PasteCurrentScaleVale();
                }
            }
        ...
        Entity nullableSelection, int rowID)
    {
        ...
        // Param References
        if (Editor.MapEditorDecorator.ParamRefRow(meta, proprow, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        // Text References
        if (Editor.MapEditorDecorator.FmgRefRow(meta, proprow, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        // Enum List
        if (Editor.MapEditorDecorator.GenericEnumRow(meta, proprow, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        // Alias List
        if (Editor.MapEditorDecorator.AliasEnumRow(meta, proprow, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        if (Editor.MapEditorDecorator.SpawnStateListRow(meta, proprow, oldval, ref newval, rowID))
        {
            changed = true;
            committed = true;
        }
    ...
    )
    {
        ...
        // Model Link
        Editor.MapEditorDecorator.ModelNameRow(meta, entSelection, prop, oldval);

        // Param References
        if (Editor.MapEditorDecorator.ParamRefRow(meta, prop, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        // Text References
        if (Editor.MapEditorDecorator.FmgRefRow(meta, prop, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        if (Editor.MapEditorDecorator.MsbReferenceRow(meta, prop, oldval, ref newval, entSelection))
        {
            changed = true;
            committed = true;
        }
        ...
        // Enum List
        if (Editor.MapEditorDecorator.GenericEnumRow(meta, prop, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        // Alias List
        if (Editor.MapEditorDecorator.AliasEnumRow(meta, prop, oldval, ref newval))
        {
            changed = true;
            committed = true;
        }
        ...
        {
            if (Editor.MapEditorDecorator.EldenRingAssetMaskAndAnimRow(meta, prop, oldval, ref newval, selection))
            {
                changed = true;
                committed = true;
            }
    ...
    )
    {
        ...
        foreach (PropertyInfo prop in properties)
        {
            ...
            else if (typ.IsClass && typ == typeof(MSB.Shape))
            {
                ...
                {
                    if (ImGui.Combo("##shapecombo", ref shap, MapEditorUtils.RegionShapes, MapEditorUtils.RegionShapes.Length))
                    {
                        MSB.Shape newshape;
            ...
            else if (typ == typeof(BTL.LightType))
            {
                ...
                if (ImGui.Combo("##lightTypecombo", ref thisType, MapEditorUtils.LightTypes, MapEditorUtils.LightTypes.Length))
                {
    ...
    private void DisplayPropertyViewDecorations(ViewportSelection selection, int refID)
    {
        ...
        if (firstEnt.References is null) 
            return;

        if (CFG.Current.MapEditor_Enable_Property_Property_SpecialProperty_Info)
        {
            Editor.MapEditorDecorator.DisplayConnectionInformation(firstEnt);
            Editor.MapEditorDecorator.DisplayConnectCollisionInformation(firstEnt);
        }
        if (CFG.Current.MapEditor_Enable_Property_Property_ReferencesTo)
        {
            Editor.MapEditorDecorator.DisplayReferenceTo(firstEnt, ref selection, ref refID);
        }
        if (CFG.Current.MapEditor_Enable_Property_Property_ReferencesBy)
        {
            Editor.MapEditorDecorator.DisplayReferenceBy(firstEnt, ref selection, ref refID);
        }
        if (CFG.Current.MapEditor_Enable_Param_Quick_Links)
        {
            Editor.MapEditorDecorator.DisplayParamLink(firstEnt, ref selection, ref refID);
        }
    }
