...
public class MapQueryView : IMapQueryEngine
{
    ...
    public MapQueryView(MapEditor editor)
    {
        Editor = editor;
    ...
    private string GetUnknownAlias(string rawName)
    {
        ...
        var chrAlias = GetAlias(rawName, Editor.Project.Aliases.Characters);
        var assetAlias = GetAlias(rawName, Editor.Project.Aliases.Characters);
        var mapPieceAlias = GetAlias(rawName, Editor.Project.Aliases.Characters);
    ...
    private string GetAlias(string name, List<AliasEntry> aliasList)
    {
        var lowerName = name.ToLower();

        if (aliasList.Any(e => e.ID == lowerName))
        {
            return aliasList.First(e => e.ID == lowerName).Name;
        }
