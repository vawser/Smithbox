...
public class PrefabView
{
    ...
    public PrefabView(MapEditor editor) 
    { 
        Editor = editor;
    }
    ...
    Prefab GetLoadedPrefab(string name)
    {
        ...
        loadedPrefab = Prefab.New(Editor.Project.ProjectType);
    ...
    void CreateFromSelection(string name)
    {
        ...
        var newPrefab = Prefab.New(Editor.Project.ProjectType);

        newPrefab.ExportSelection($@"{prefabDir}\{name}.json", name, editFlags, Editor.Selection);
    ...
    {
        bool selectedEntities = Editor.Selection.GetFilteredSelection<MsbEntity>().Any();
    ...
    {
        prefabs = new();
        prefabDir = $"{Editor.Project.ProjectPath}\\.smithbox\\{MiscLocator.GetGameIDForDir()}\\prefabs\\";
