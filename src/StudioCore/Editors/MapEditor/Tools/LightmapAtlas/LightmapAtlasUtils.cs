...
{
    public static void DuplicateEntries(MapEditorScreen Screen, LightmapAtlasView AtlasScreen)
    {
        ...
        else
        {
            ...
            if (lastIdx != -1)
            {
                SelectNewEntry(lastIdx);
            }
        }
    }
    public static void DeleteEntries(MapEditorScreen Screen, LightmapAtlasView AtlasScreen)
    {
        if (AtlasScreen.CurrentParent == null)
            return;
    ...
    {
        var handler = Smithbox.EditorHandler.MapEditor.LightmapAtlasView;
