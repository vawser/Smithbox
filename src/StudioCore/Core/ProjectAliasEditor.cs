...
public static class ProjectAliasEditor
{
    ...
    private static void DisplayAliasEditor()
    {
        ...
        for (int i = 0; i < CurrentAliasEntry.Tags.Count; i++)
        {
            ...
            if (ImGui.Button($"{ForkAwesome.Minus}##tagRemove{i}"))
            {
            ...
            if (ImGui.Button($"{ForkAwesome.Plus}##tagAdd{i}"))
            {
    ...
    private static void Shortcuts()
    {
        ...
        if (TargetProject.ActionManager.CanUndo() && InputTracker.GetKey(KeyBindings.Current.CORE_UndoContinuousAction))
        {
