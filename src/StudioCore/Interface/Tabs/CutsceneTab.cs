...
public class CutsceneTab
{
    ...
    public void Display()
    {
        ...
        if (CutsceneAliasBank.Bank.CanReloadBank)
        {
            CutsceneAliasBank.Bank.CanReloadBank = false;
    ...
    public void DisplayAddSection()
    {
        ...
        if (ImGui.Button("Add New Alias"))
        {
            ...
            if (_newRefId != "")
            {
                ...
                var entries = CutsceneAliasBank.Bank.AliasNames.GetEntries("Cutscenes");
                ...
                if (isValid)
                {
                    CutsceneAliasBank.Bank.AddToLocalAliasBank("", _newRefId, _newRefName, _newRefTags);
                    ImGui.CloseCurrentPopup();
                    CutsceneAliasBank.Bank.CanReloadBank = true;
                }
                else
                {
    ...
    public void DisplayEntryList()
    {
        ...
        ImGui.SameLine();
        ImGui.Checkbox("Show Tags", ref CFG.Current.CutsceneAtlas_ShowTags);
        ...
        DisplaySelectionList(CutsceneAliasBank.Bank.AliasNames.GetEntries("Cutscenes"));
    ...
    private void DisplaySelectionList(List<AliasReference> referenceList)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            if (CFG.Current.CutsceneAtlas_ShowTags)
            {
    ...
    private void DisplayEditWindow()
    {
        if (_selectedEntry != null)
        {
            ...
            {
                CutsceneAliasBank.Bank.AddToLocalAliasBank("", _refUpdateId, _refUpdateName, _refUpdateTags);
                ImGui.CloseCurrentPopup();
                CutsceneAliasBank.Bank.CanReloadBank = true;
            }
            ImGui.SameLine();
            ...
            {
                CutsceneAliasBank.Bank.RemoveFromLocalAliasBank("", _refUpdateId);
                ImGui.CloseCurrentPopup();
                CutsceneAliasBank.Bank.CanReloadBank = true;
            }
            ImGui.SameLine();
