...
// Rumble animation HKX within other/ folder
// <x>.rumblebnd.dcx
// -> INTERROOT_win64/other/rumble/camera_<xxx>.hkx

public class HavokRumbleBank : IDisposable
{
    ...
    {
        foreach (var entry in Project.Locator.HavokRumbleFiles.Entries)
        {
            PopulateFiles(entry);
        }
    ...
    public void LoadRumbleFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
                ...
                using (MemoryStream memoryStream = new MemoryStream(fileBytes))
                {
                    ...
                    catch (InvalidDataException ex)
                    {
                        Smithbox.LogError(this,
                            LOC.Get("HAVOK_Data_Failed_to_Read_Rumble_HKX", name), ex);
                    }
                }
            }
            catch (Exception ex)
            {
                Smithbox.LogError(this,
                    LOC.Get("HAVOK_Data_Failed_to_Read_Rumble_Binder_File", name), ex);
            }
        }
    }
    ...
    public void SaveRumbleFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
            ...
            {
                Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Rumble_HKX", name), ex);
            }
        }
        ...
        {
            Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Rumble_HKX", fileEntry.Path), ex);
        }
    }
    #region
    public void Dispose()
