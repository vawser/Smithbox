...
// Part collidable HKX within parts/ folder
// wp_a_<xxxx>.partsbnd.dcx
// -> WP_A_<xxxx>_collidable.hkx

public class HavokPartBank : IDisposable
{
    public ProjectEntry Project;

    public Dictionary<HavokFileLocation, hkRootLevelContainer> Files = new();

    public HavokPartBank(ProjectEntry project)
    {
        Project = project;
    }
    public async Task<bool> Setup()
    {
        foreach (var entry in Project.Locator.HavokPartFiles.Entries)
        {
            PopulateFiles(entry);
        }
    ...
    public void LoadPartFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
                ...
                using (MemoryStream memoryStream = new MemoryStream(fileBytes))
                {
                    ...
                    catch (InvalidDataException ex)
                    {
                        Smithbox.LogError(this,
                            LOC.Get("HAVOK_Data_Failed_to_Read_Part_HKX", name), ex);
                    }
                }
            }
            catch (Exception ex)
            {
                Smithbox.LogError(this,
                    LOC.Get("HAVOK_Data_Failed_to_Read_Part_Binder_File", name), ex);
            }
        }
    }
    ...
    public void SavePartFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
            ...
            {
                Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Part_HKX", name), ex);
            }
        }
        ...
        {
            Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Part_HKX", fileEntry.Path), ex);
        }
    }
    #region
    public void Dispose()
