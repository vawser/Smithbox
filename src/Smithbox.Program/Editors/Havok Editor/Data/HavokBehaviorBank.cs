...
// Behavior HKX within chr/ folder
// <x>.behbnd.dcx
public class HavokBehaviorBank : IDisposable
{
    ...
    public HavokBehaviorBank(ProjectEntry project)
    {
        Project = project;
    }
    ...
    public void LoadBehaviorFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
                ...
                using (MemoryStream memoryStream = new MemoryStream(fileBytes))
                {
                    ...
                    catch (InvalidDataException ex)
                    {
                        Smithbox.LogError(this,
                            LOC.Get("HAVOK_Data_Failed_to_Read_Behavior_HKX", name), ex);
                    }
                }
            }
            catch (Exception ex)
            {
                Smithbox.LogError(this,
                    LOC.Get("HAVOK_Data_Failed_to_Read_Behavior_Binder_File", name), ex);
            }
        }
    }
    ...
    public void SaveBehaviorFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
            ...
            {
                Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Behavior_HKX", name), ex);
            }
        }
        ...
        {
            Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Behavior_HKX", fileEntry.Path), ex);
        }
    }
    #region
    public void Dispose()
