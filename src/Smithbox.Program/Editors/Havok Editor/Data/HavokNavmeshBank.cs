...
// Navmesh HKX within map/m<xx>/m<xx_xx_xx_xx>/ folders
// <x>.nvmhktbnd.dcx
public class HavokNavmeshBank : IDisposable
{
    ...
    {
        foreach (var entry in Project.Locator.HavokNavmeshFiles.Entries)
        {
            PopulateFiles(entry);
        }
    ...
    public void LoadNavmeshFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
                ...
                using (MemoryStream memoryStream = new MemoryStream(fileBytes))
                {
                    ...
                    catch (InvalidDataException ex)
                    {
                        Smithbox.LogError(this,
                            LOC.Get("HAVOK_Data_Failed_to_Read_Navmesh_HKX", name), ex);
                    }
                }
            }
            catch (Exception ex)
            {
                Smithbox.LogError(this,
                    LOC.Get("HAVOK_Data_Failed_to_Read_Navmesh_Binder_File", name), ex);
            }
        }
    }
    ...
    public void SaveNavmeshFile(FileDictionaryEntry fileEntry, string internalFilePath)
    {
        ...
        foreach (var file in binder.Files)
        {
            ...
            {
                Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Navmesh_HKX", name), ex);
            }
        }
        ...
        {
            Smithbox.LogError(this, LOC.Get("HAVOK_Data_Failed_to_Write_Navmesh_HKX", fileEntry.Path), ex);
        }
    }
    #region
    public void Dispose()
