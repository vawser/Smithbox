using Andre.Formats;
using global::StudioCore.Application;
using global::StudioCore.Editors.Common;
using global::StudioCore.Editors.MapEditor;
using global::StudioCore.Editors.ParamEditor;
using global::StudioCore.Utilities;
...
public static class ParamMapReferences
{
    // Supports: ER, DS3, SDT
            ...
            if (assetID != "")
            {
                foreach (var entry in AssetList)
                {
                    if (entry.Filename.Equals(assetID, System.StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        GUI.WideTooltip($"{assetID}: {aliasName}");
                    }
                }
            }
        }
